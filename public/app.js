...
form.addEventListener('submit', async event => {
  ...
  const payload = { url: data.get('url'), dismissSelector: data.get('dismissSelector') || undefined, viewport: { width: Number(data.get('width')), height: Number(data.get('height')) }, figmaUrl: data.get('figmaUrl') || undefined, figmaToken: data.get('figmaToken') || undefined, visualThreshold: Number(data.get('visualThreshold')), baseline: data.get('baseline') === 'on', flow, tests };
